...
class TripletMixture:
    def create_mix(self, idx, triplet, snr_levels, out_dir, test=False, sr=16000, **kwargs):
        trim_db, vad_db = kwargs["trim_db"], kwargs["vad_db"]
        audio_len = kwargs["audioLen"]

        s1, s2, ref = self._read_files(triplet)

        meter = pyln.Meter(sr)  # create BS.1770 meter

        s1_norm = self._normalize_loudness(meter, s1, -29)
        s2_norm = self._normalize_loudness(meter, s2, -29)
        ref_norm = self._normalize_loudness(meter, ref)

        if self._silence_check(s1_norm, s2_norm, ref_norm):
            return
        ...
        if not test:
            ...
            for i in range(len(s1_cut)):
                ...
                s1_cut[i] = self._normalize_loudness(meter, s1_cut[i])
                mix = self._normalize_loudness(meter, mix)

                self._write_files(mix, s1_cut[i], ref, sr, idx, triplet, out_dir, cut=i)

        else:
            s1, s2 = fix_length(s1, s2, 'max')
            mix = snr_mixer(s1, s2, snr)

            s1 = self._normalize_loudness(meter, s1)
            mix = self._normalize_loudness(meter, mix)

            self._write_files(mix, s1, ref, sr, idx, triplet, out_dir)

    @staticmethod
    def _read_files(triplet):
    ...
    @staticmethod
    def _write_files(mix, target, ref, sr, idx, triplet, out_dir, cut=None):
